...
function getFizzBuzz(num) {
  const isMultOfThree = num % 3 === 0;
  const isMultOfFive = num % 5 === 0;

  if (isMultOfFive && isMultOfThree) return 'FizzBuzz';
  if (isMultOfThree) return 'Fizz';
  if (isMultOfFive) return 'Buzz';
  return num;
}
...
function isCreditCardNumber(ccn) {
  ...
  return digits.map((item, index) => {
    let res = +item;
    if (((index % 2) && (digits.length % 2)) || (!(index % 2) && !(digits.length % 2))) {
      res = (item * 2 < 9) ? item * 2 : item * 2 - 9;
    }
    return res;
...
function getDigitalRoot(num) {
  ...
  if (digits.length < 2) {
    ...
    res = getDigitalRoot(digits.reduce((sum, item) => sum + +item, 0));
  }
  return res;
}
...
function getCommonDirectoryPath(pathes) {
  ...
  while (match) {
    for (let i = 1; i < pathes.length; i += 1) {
      match = match && (pathes[0][currCnt] === pathes[i][currCnt]);
    }
    if (match) outStr += pathes[0][currCnt];
    currCnt += 1;
  }
...
function getMatrixProduct(m1, m2) {
  ...
  return res
    .map((row, i) => row.map((_val, j) => m1[i].reduce((sum, el, k) => sum + (el * m2[k][j]), 0)));
}
